<template>
  <div class="buss-detail" v-if="detailInfo.businessId">
    ...
    <div class="c-body">
      ...
      <p>报销类型：<span>{{detailInfo.feeTerm}}</span></p>
      <p>报销公司：<span>深圳市富诚担保有限公司</span></p>
      <p>报销部门：<span>法务中心</span></p>
      ...
      <p>付款类型：<span>{{detailInfo.feeTerm}}</span></p>
      <p>付款金额(元)：<span>{{detailInfo.feeMoney}}（{{detailInfo.feeMoney4Big}}）</span></p>
      <p>付款方式：<span>{{detailInfo.tradeType}}</span></p>
      <p>用途：<span>{{detailInfo.payReason}}</span></p>
      <p>备注：<span>{{detailInfo.remarks}}</span></p>
      <p>情况说明：<span>{{detailInfo.conditionExplain}}</span></p>
      <img-list :imgList="imgList" :isModify="false"></img-list>
      <!-- v-if="detailInfo.processStatus4DD=='0'" -->
      <div v-if="detailInfo.processStatus4DD=='0'">
        <p>审批结果：
          <x-switch style="display: inline-block;vertical-align: middle;" title="" v-model="isAgree"></x-switch>
          {{isAgree?'同意':'拒绝'}}
        </p>
        <p>审批备注：
